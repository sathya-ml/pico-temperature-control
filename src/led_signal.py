class LedTemperatureSignaller:
    def __init__(self, attached_led, target_temperature, max_tolerance, blinking_interval=250):
        ...
        Args:
            attached_led (object): An object with `turn_on()`, `start_blinking()`, and `turn_off()` methods to
                control the LED.
            target_temperature (float): The desired temperature that the system aims to maintain.
            max_tolerance (float): The maximum allowable deviation from the target temperature before changing the
                LED signal.
            blinking_interval (int, optional): The interval in milliseconds for blinking when the temperature exceeds
                the maximum bound. Defaults to 250.
        """
